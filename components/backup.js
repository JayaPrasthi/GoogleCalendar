...
export const GoogleCalender = () => {
     ...
     .then((response) => {
         response.forEach(res=>{
           ...
           if(startTimeStampElement.id.includes(res.start_time) && endTimeStampElement.id.includes(res.end_time)){
            for(let i = res.start_time; i <= res.end_time; i++){ // for loop can also be used to get the range or get the in between values 
    ...
    //                 <div id="weekday" className="text-[#70757a] text-[11px] font-[500]  "> {key} </div>
    //                 <div className="text-[26px] text-[#70757a] rounded-[100%]">  {value} </div>
    ...
    let splitedArr = []
    let pureNumArr = []
    const handleClick = (event, item, item1) => { // only after click event happenes the moveevent will happen
        // console.log("clickevent", event)
        let IndividualRow = document.getElementById(`day-${item}`) // Taking the row instead of the cell cause after moving into individaul cell click event should happen
        // console.log("A",arr1)
    ...
    const handleMouseMove = (event, item, item1) => {
        ...
        // console.log("arr",arr)
        // console.log("mousemoveevent", event.target)
    ...
    const handleMouseup = (item) => {
        ...
        // console.log("pp",IndividualRow)
        // IndividualRow.removeEventListner("mousemove", handleMouseMove)
        let IndividualRow = document.getElementById(`day-${item}`)
        if (arr1 !== undefined || arr1 !== null) {
            if (arr1[arr1.length - 1] !== arr1[0]) {
                ...
                // console.log("l",lastValueofArr1)
                lastValueofArr1.parentElement.removeEventListener("mousemove", handleMouseMove)
                setOpen(true)
                // console.log(typeof(LastValueofArr1))

            }
            // else if(arr1[arr1.length-1]==arr1[0]){
            //     var FirstValueofArr1 = arr1[0]
            //     let firstValueofArr1 =  document.getElementById(FirstValueofArr1)
            //     firstValueofArr1.parentElement.removeEventListener("mousemove", handleMouseMove)
            // }
            ...
            if (arr2) {
                ...
                if (splitedArr.forEach(item => {
                    ...
                    // console.log("purNum", pureNum)
                    pureNumArr.push(pureNum)

                }))
                    // pureNumArr.sort((a,b)=>a-b)
                    // console.log("pureNumArr", pureNumArr)
                ...
                // console.log("split",splitedArr)


                // let splicedArray = splitArr.splice(0,14)
                // console.log("SS", splicedArray)
            ...
            // console.log("Individual", IndividualRow)
...
//     }
    ...
    const handlePrevious = (event) =>{
        ...
        for(let i=1;i<=7;i++){
            let prevWeekObj={}
            let DateofthePrevWeek = firstDate.Date - i // gets the values of the date in the week  
            let CompeleteDateOfTheWeek =  new Date(date.setDate(DateofthePrevWeek)) // get the complete date of the week 
            // console.log("CompleteDateoftheWeek", new Date(CompeleteDateOfTheWeek))
            let DayofthePrevWeek = CompeleteDateOfTheWeek.toString().slice(0,4) // gets the day of the week 
            // console.log("Date",DateofthePrevWeek,"Day", DayofthePrevWeek)
            prevWeekObj["Day"]=DayofthePrevWeek  // create object key for Day 
            prevWeekObj["Date"]=DateofthePrevWeek // create object key for the Date 
            // console.log("prev", prevWeekObj)
            tempArr2.push(prevWeekObj) // pushing each object into the array 
            
            
            
            if(DateofthePrevWeek == 1){
                let nextMonth = (date.getMonth()+1)-1
                // console.log("nextMonth", nextMonth)
                var lastDayOftheMonth = new Date(date.getFullYear(), nextMonth, 0); // 0 means one day less than first day of the month which is last day of the previous month.
                console.log("lastDayOfMonth", lastDayOftheMonth)
        ...
        // console.log("tempArr2", tempArr2.Date)
        // console.log("tempArr2", typeof(tempArr2[0].Date))
        ...
        console.log("DayItemsofPrev", DayItems)
        // location.reload()
    ...
    const handleNext = (event) =>{
        ...
        for(let i=1;i<=7;i++){
            ...
            // console.log("DateofNextWeek",DateOftheNextWeek)
